...
Eq. (1.1),
ˆ ˆ B A r = r ?R = l r? ? l r
? ? ? . (1.5)
We can decompose each of the unit vectors ˆr and ˆ? r with respect to the Cartesian unit vectors ˆi
and ˆj (see figure)
rˆ = ?sin? ˆi + cos? ˆj (1.6)
rˆ? = sin? ˆi + cos? ˆj . (1.7)
Then Eq. (1.5) giving the position vector of particle b in frame B becomes
ˆ ˆ (sin ˆ cos ˆ) ( sin ˆ cos ˆ) 2 sin ˆ B r? = l r? ? l r = l ? i + ? j ? l ? ? i + ? j = l ? i . (1.8)
In order to find the velocity vector of particle a in frame B (i.e. with respect to particle b),
differentiate Eq. (1.8)
(2 sin ) ˆ (2 cos ) ˆ 2 cos ˆ B
